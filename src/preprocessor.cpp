...
void ConvertRgbToNchwScalar(const cv::Mat& rgb_image,
...
void ConvertRgbToNchwOpenMP(const cv::Mat& rgb_image,
                            std::vector<float>& tensor,
                            const int width,
                            const int height,
...
PreprocessResult ImagePreprocessor::Run(const cv::Mat& bgr_image) const
{
    ...
    if (mode_ == PreprocessMode::OpenMP) {
        ...
        ConvertRgbToNchwOpenMP(rgb_image, result.tensor, input_width_, input_height_, thread_count);
    } else {
        ...
        ConvertRgbToNchwScalar(rgb_image, result.tensor, input_width_, input_height_);
    }
